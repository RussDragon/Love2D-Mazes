...
function love.load(arg)
  ...
  local t1 = os.clock()
 	-- mGrid, changes_mat = recursivedivision.createMaze(1, 1, columns, rows, 2)
...
function love.update(dt)
	if Changes_Stage == 1 then
		love.timer.sleep(1)
	else
		love.timer.sleep(0.15)
	end

	if isAnimated and changes_mat and Changes_Stage < #changes_mat then 
		Changes_Stage = Changes_Stage + 1 
	end
end
...
function love.draw()
	local grid
	if isAnimated then
		grid = changes_mat[Changes_Stage]
	else grid = mGrid end
