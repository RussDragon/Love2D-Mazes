...
function mod.divideGrid(grid, x1, y1, x2, y2)
  if y2 - y1 > 2 and x2 - x1 > 2 then
    ...
    if x2-x1 > y2-y1 then 
      ...
      mod.divideGrid(grid, x1 + 1, y1 + 1, ox - 1, y2 - 1)
      mod.divideGrid(grid, ox + 1, y1 + 1, x2 - 1, y2 - 1)

      else if x2-x1 == y2-y1 then
        if math.random(0,1) == 0 then
          aux.add_horizontal_wall(grid, x1, x2, oy) 
          mod.divideGrid(grid, x1 + 1, y1 + 1, x2 - 1, oy - 1)
          mod.divideGrid(grid, x1 + 1, oy + 1, x2 - 1, y2 - 1)
        else 
          aux.add_vertical_wall(grid, y1, y2, ox)
          mod.divideGrid(grid, x1 + 1, y1 + 1, ox - 1, y2 - 1)
          mod.divideGrid(grid, ox + 1, y1 + 1, x2 - 1, y2 - 1) 
